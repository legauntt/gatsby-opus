"""Build the Tony AI catalog from the local approved MP3 library."""
...
def prepare(path):
    ...
    return {'id':slug,'title':path.stem,'original_filename':path.name,
            'url':BASE+name,'duration':duration,'bytes':path.stat().st_size,'sha256':digest,
...
public['tracks']=[{k:v for k,v in t.items() if k not in ['original_filename','sha256','sample_rate','channels']} for t in tracks]
